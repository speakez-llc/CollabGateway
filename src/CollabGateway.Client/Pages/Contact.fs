...
let private update (msg: Msg) (model: State) (parentDispatch: ViewMsg -> unit) : State * Cmd<Msg> =
    ...
    | SubmitForm ->
        ...
        if hasErrors then
            ...
            let cmd = Cmd.OfAsync.eitherAsResult (fun _ -> service.ProcessContactForm (timeStamp, sessionToken, model.ContactForm)) FormSubmitted
